$(function(){
    ...
    var defaultEvents = {
        ...
        events: [
            {   title: 'Product release party',
                ...
                guests: ['Albert Camus', 'Nietsche', 'Kate Klover'],
                ...
                guests: ['Hans Zimmer', 'Roberto Rodriguez', 'Dave Malmstein'],
                ...
                guests: ['David Copperfield', 'Raza Kiostori'],
    ...
    var calendar = $('#calendar').fullCalendar({
        ...
        eventRender: function(event, element) {
          ...
          if(event.details) {
            element.find('.fc-list-item-title').append('<div class="fc-details">' + event.details + '</div>');
            element.find('.fc-content').append('<div class="fc-details">' + event.details + '</div>');
          }
        },
        select: function (start, end, allDay) {
            ...
            create_event_form.find("input[name='start']").val(start.format('LL'));
            create_event_form.find("input[name='end']").val(end.format('LL'));
            ...
            $(create_modal).on('hidden.bs.modal', function () {
              ...
              var title = create_event_form.find("input[name='event-title']").val(),
                  categoryClass = create_event_form.find("select[name='category'] option:checked").val();
        ...
        eventClick: function (calEvent, jsEvent, view) {
            ...
            view_modal.find('.modal-title').text(calEvent.title);

            if(calEvent.details && calEvent.details.length) {
              view_modal.find('.details').text(calEvent.details);
              // view_modal.find('.details').removeClass('d-none');
              }  else {
                  view_modal.find('.details').text('No details found! Sorry.');
              }

            view_modal.find('.event-starts').text(moment(calEvent.start).format('LLL'));

            calEvent.end = (calEvent.end) ? moment(calEvent.end).format('LLL') : 'Ends today';
            view_modal.find('.event-ends span').text(calEvent.end);

            view_modal.find('.event-location span').text(calEvent.location);

            if(calEvent.details && calEvent.details.length) {
              view_modal.find('.event-location span').text(calEvent.location);
              }  else {
                  view_modal.find('.event-location span').text('N/A');
              }
            ...
            if(guests) {
              ...
              view_modal.find('.guests-list').html(guest_list);
              view_modal.find('.guests').removeClass('d-none');
            } else {
                view_modal.find('.guests').addClass('d-none');
            }

            view_modal.find('.delete_event').on('click', function() {
